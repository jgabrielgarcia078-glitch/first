...
- `strikezone.html` — **StrikeZone** (v1.1), FPS tático 5 contra 5 inspirado no CS:GO (mapa `de_duna`, desarme de bomba, economia, bots, molotov/incendiária, rádio do time, relatório de dano, placar com ADR/HS%) + modo **Treino de mira**. Independente do GoalQuest: não compartilha código nem dados. Segue as mesmas 7 regras de estrutura (arquivo único, IIFE, sem handlers inline, constantes/estado no topo, um único `init` no `DOMContentLoaded`). WebGL 2 puro sem bibliotecas (materiais procedurais assados uma vez numa texture array, objetos dinâmicos instanciados, sombra estática + sombra dinâmica pequena, resolução interna configurável), áudio sintetizado com WebAudio, `localStorage` só para preferências (`strikezone.settings.v2`, migra da v1). Mira desenhada em canvas alinhado ao pixel (estilos, cor, espessura, contorno, ponto; por padrão **não abre ao atirar**). A mira do mouse não tem suavização: tranco de câmera (`viewRecoil`), balanço da arma (`vmBob`) e soco ao levar tiro (`HIT_PUNCH`) são pequenos de propósito — não aumentar sem pedido do usuário. Abrir com `?debug=1` expõe `window.__SZ` (estado, `simulate(segundos)`, `play('match'|'practice')`, `set(chave, valor)`, `findPath`, `losClear`, `buy`, `vmPose`) para testes com Playwright.
