...
- `strikezone.html` — **StrikeZone** (v1.2), FPS tático 5 contra 5 inspirado no CS:GO (mapa `de_duna` com containers, escadas de mão e caixotes de pulo agachado; desarme de bomba, economia, bots, molotov/incendiária, rádio do time, relatório de dano, placar com ADR/HS%) + **Mata-mata em equipe** + **Treino de mira** com exercícios (livre, flick, só cabeça, tracking, reflexo) e recordes. Independente do GoalQuest: não compartilha código nem dados. Segue as mesmas 7 regras de estrutura (arquivo único, IIFE, sem handlers inline, constantes/estado no topo, um único `init` no `DOMContentLoaded`). WebGL 2 puro sem bibliotecas (materiais procedurais e pinturas de arma assados uma vez numa texture array, oclusão de ambiente assada por vértice, objetos dinâmicos instanciados, sombra estática + sombra dinâmica pequena, bloom opcional em buffer MSAA, resolução interna configurável), áudio sintetizado com WebAudio (som 3D HRTF opcional), `localStorage` só para preferências (`strikezone.settings.v2`, migra da v1) e recordes do treino (`strikezone.records.v1`).
  - Tiro: padrões de spray fixos por arma (`SPRAY_PATTERNS`), precisão total abaixo de `ACCURATE_SPEED_FRAC` da velocidade (counter-strafe), penetração por material (`MAT_PEN`), tag por classe de arma (`TAG_SLOW`). Mira desenhada em canvas alinhado ao pixel (por padrão **não abre ao atirar**). A mira do mouse não tem suavização: tranco de câmera (`viewRecoil`), balanço da arma (`vmBob`) e soco ao levar tiro (`HIT_PUNCH`) são pequenos de propósito — não aumentar sem pedido do usuário.
  - Bots: personalidades (`BOT_PERSONAS`), economia de time (pistol/eco/force/full, drops para colegas), granadas com posições fixas (`NADE_LINEUPS`; força e inclinação calculadas por simulação em `solveThrow`), retake com ponto de encontro (`RETAKE_RALLY`), rotação por sinais de TR perto de cada bomb, comandos de rádio do jogador na tecla Z (`RADIO_COMMANDS`).
  ...
  - Abrir com `?debug=1` expõe `window.__SZ` (estado, `simulate(segundos)`, `play('match'|'dm'|'practice', exercício)`, `set(chave, valor)`, `issueRadioCommand`, `solveThrow`, `findPath`, `losClear`, `buy`, `vmPose`) e `game.debugLog` (granadas dos bots) para testes com Playwright.
